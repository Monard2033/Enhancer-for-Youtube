(function () {
    ...
    styleElement.textContent = `
        :root {
          ...
          --light-bt: rgba(0, 0, 0, 0.05);
          --light-bt-hover: rgba(0, 0, 0, 0.1);
        }
        #start.ytd-masthead {
          height: 50px;
          border-radius: 30px;
    ...
    function updateLayout() {
      ...
      if (masthead && center && container) {
        ...
        // Interpolate opacity based on windowWidth: 0.6 at 1035px, 1 at 1920px
        container.style.opacity = window.scrollY !== 0 ? '0.6' : '1';
        let centerFlexBasis = 200 + (windowWidth - 1035) * 0.3955;
        centerFlexBasis = Math.max(200, Math.min(550, centerFlexBasis)); // Clamp between 200 and 550
      ...
      if (primaryElement && columnsElement) {
        ...
        if (window.screen.width === 2560 && window.screen.height === 1440) {
          ...
          maxWidthValue = 1850;
          // Linear interpolation: position = 0.8633 * (viewportWidth - 1035) + 80
          position = 0.8633 * (viewportWidth - 1035) + 80;
          // Clamp position between 80px and 854px
